    ...
    if mode == "selected":
        with open(rule_file, "r") as fi:
            while True:
                ...
                if not line:
                    break
                r, num_lines = [int(_) for _ in line.strip().split()]
                for k in range(num_lines):
                    line = fi.readline()
                    items = line.strip().split()
                    rule = (int(items[1]), [int(items[i]) for i in range(3, len(items))])
                    rules.append(rule)
    elif mode == "all":
        with open(args.rule_file, "r") as fi:
            for line in fi:
                items = line.strip().split()
                r_head = int(items[0])
                r_body = [int(x) for x in items[1:]]
                rule = (r_head, r_body)
                rules.append(rule)
    elif mode == "sorted":
        with open(args.rule_file, "r") as fi:
            r2rules = [[] for r in range(self.kg.relation_size)]
            for line in fi:
                items = line.strip().split()[1:]
                r_head = int(items[0])
                r_body = [int(x) for x in items[1:]]
                rule = (r_head, r_body)
                r2rules[r_head].append(rule)
            for r in range(self.kg.relation_size):
                ed = min(100, len(r2rules[r]))
                rules += r2rules[r][:ed]
    else:
        raise ValueError
    ...
    def H_score(self, dataset):
        ...
        for r in range(self.num_relations):
            ...
            for index, score in r2index_score[r]:
                rule = [len(self.rules[index][1]), self.rules[index][0]] + self.rules[index][1] + [score]
